...
@app.route('/')
def index():
    # with open('configs.py','r') as config:
    #     config.read()
    # print(config)
...
def get_weather():
    ...
    url="https://api.weatherapi.com/v1/forecast.json?key=%s&q=%s,%s&days=%s&aqi=no&alerts=no"%(weather_key,latitude,longitude,3)
    response = requests.get(url)
    weather = response.json()

    weather_dataset = {"current":{'last_updated':"","temp_c":"","condition":{"name":"","icon":""}},
    "days":[]}
